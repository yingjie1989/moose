...
validParams<MechanicalContactConstraint>()
{
  ...
  params.addParam<Real>(
      "penetration_tolerance", 1e-9, "The tolerance of the distance function");
  params.addParam<Real>(
          "stickking_tolerance", 1e-9, "The tolerance of the sticking");

  params.addParam<Real>(
                  "frictionalforce_tolerance", 1e-9, "The tolerance of the frictionalforce");
         ...
         if (_model == CM_COULOMB){
          ...
          for (unsigned int i = 0; i < _mesh_dimension; ++i)
          {
            ...
            const Real capacity(_friction_coefficient *
                                ((res_vec * pinfo->_normal - pinfo->_lagrange_multiplier ) > 0 ? (res_vec * pinfo->_normal - pinfo->_lagrange_multiplier ) : 0));*/


           RealVectorValue contact_force_normal( ( pinfo->_lagrange_multiplier + (pen_force * pinfo->_normal) ) * pinfo->_normal );
          ...
          //   pinfo->_lagrange_multiplier = pinfo->_lagrange_multiplier*0.0;
          //   pinfo->_lagrange_multiplier_slip = pinfo->_lagrange_multiplier_slip*0.0;
         }
   }
...
MechanicalContactConstraint::contactConverged() //const NumericVector<Number> & solution)
{
  ...
  //The above piece of code could be removed
    ...
    if (_model == CM_COULOMB){
          ...
          Real distance_val = distance_vec * pinfo->_normal;

          if (contactResidual < std::abs(distance_val) )
                   contactResidual = std::abs(distance_val);
...
//  _console << "normal lagrangian multiplier is %4.3f "<< pinfo->_lagrange_multiplier <<"\n";
//  _console << "slip lagrangian multiplier is %4.3f "<< pinfo->_lagrange_multiplier_slip(0) <<"\n";
      ...
      switch (_model)
      {
        ...
        case CM_COULOMB:
        {
          ...
          else if ( _formulation == CF_AUGMENTED_LAGRANGE )
          {
            ...
            if  (pinfo->_mech_status == PenetrationInfo::MS_SLIPPING || pinfo->_mech_status == PenetrationInfo::MS_SLIPPING_FRICTION)
                 return normal_comp;
            ...
            RealVectorValue tan_slip_dir = tangential_inc_slip / tangential_inc_slip.norm();
        ...
        case CM_COULOMB:
          switch (_formulation)
          {
            ...
            case CF_AUGMENTED_LAGRANGE:
            {
              ...
              /*else if (pinfo->_mech_status == PenetrationInfo::MS_STICKING)
              {
                    RealVectorValue tangential_inc_slip =
                        pinfo->_incremental_slip -
                        (pinfo->_incremental_slip * pinfo->_normal) * pinfo->_normal;
                    ...
                    return - _phi_slave[_j][_qp] * penalty * _test_master[_i][_qp] *
                             pinfo->_normal(_component) * normal_component_in_coupled_var_dir
                           - _phi_slave[_j][_qp] * penalty_slip * _test_master[_i][_qp] *
                              tan_slip_dir(_component) * tan_component_in_coupled_var_dir;
              }*/
                return 0.0;
            }
      ...
      switch (_model)
      {
        ...
        case CM_COULOMB:
        {
          if (_formulation == CF_AUGMENTED_LAGRANGE )
          {
                ...
                Real tang_comp = _phi_master[_j][_qp] * penalty_slip * _test_master[_i][_qp] *
                          tan_slip_dir(_component) * tan_component_in_coupled_var_dir;

                return normal_comp + tang_comp;*/

          }else if (_formulation == CF_PENALTY && (pinfo->_mech_status == PenetrationInfo::MS_SLIPPING ||
               pinfo->_mech_status == PenetrationInfo::MS_SLIPPING_FRICTION))
